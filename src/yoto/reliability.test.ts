  ...
  await assert.rejects(
    getAccessToken({
      ...
      tokenStore: {
        read: async () => tokens,
        write: async () => {
          assert.fail("must not overwrite tokens");
        },
        clear: async () => {
          assert.fail("must not delete tokens");
        },
      },
      fetch: async () => {
        calls++;
...
test("upload preflight rejects an empty later track before any remote requests", async (t) => {
  ...
  const first = path.join(directory, "first.mp3"),
    second = path.join(directory, "empty.mp3");
  ...
  await assert.rejects(
    uploadPlaylist({
      ...
      tracks: [
        ...
        { filePath: second, title: "Empty", order: 2 },
      ],
      getAccessToken: async () => {
        assert.fail("must not request auth");
      },
      fetch: async () => {
        assert.fail("must not contact Yoto");
      },
    }),
    ...
    const operation = uploadPlaylist({
      ...
      fetch: async (input) => {
        ...
        if (String(input).includes("/transcoded")) {
          polls++;
          if (polls === 1)
            return new Response(null, { status: 429, headers: { "Retry-After": delay } });

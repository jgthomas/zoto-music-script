...
export async function checkUploadRecovery(options: {
  jobStore?: UploadJobStore; jobKey?: string; newCopy?: boolean; retryCreate?: boolean;
}): Promise<void> {
  ...
  if (job?.creating && !job.cardId && !options.retryCreate) {
    throw new Error("The previous Yoto playlist creation outcome is unknown. Check your Yoto library. " +

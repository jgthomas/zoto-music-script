...
export async function request(
  fetchImpl: typeof fetch, input: string | URL, init: RequestInit,
  operation: string, timeoutMs = 30_000,
): Promise<Response> {
  ...
  try {
    return await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        timer = setTimeout(() => {
          controller.abort();
          reject(new Error(`${operation} timed out. Check your connection and resume later; creation outcomes may be uncertain.`));
        }, timeoutMs);
      }),
    ]);
  } catch {
    if (controller.signal.aborted) {
      throw new Error(`${operation} timed out. Check your connection and resume later; creation outcomes may be uncertain.`);
    }
    throw new Error(`${operation} could not reach Yoto or read its response. Check your connection and resume later.`);
  } finally {
    clearTimeout(timer);
  }
}
...
export async function responseError(response: Response, operation: string): Promise<YotoHttpError> {
  ...
  } catch { /* HTML and unstructured bodies are intentionally not printed. */ }
  ...
  if (code === "invalid_grant" || response.status === 401) {
    ...
    hint = "Access was denied. Check the application's scopes and your permission to access this content.";
  } else if (response.status === 429) {
    const delay = retryAfterMs(response);
    hint = delay === undefined ? "Rate limited. Wait before resuming." : `Rate limited. Wait at least ${Math.ceil(delay / 1000)} seconds before resuming.`;
  } else if (response.status >= 500) {
    hint = "Yoto is temporarily unavailable. Resume later.";
  }
  return new YotoHttpError(response.status, `${operation} failed (${response.status}). ${hint}`, deleted);
}
...
export async function readJson<T>(response: Response, operation: string): Promise<T> {
  try {
    ...
    throw new Error(`${operation}: Yoto returned an invalid JSON response. Saved progress is retained.`);
  }
}

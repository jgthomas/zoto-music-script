...
test("HTTP errors classify authentication, permission, rate limits and deleted content safely", async () => {
  for (const [status, payload, expected] of [
    ...
    const error = await responseError(
      Response.json(payload, { status, headers: { "Retry-After": "8" } }),
      "Request",
    );
  ...
  assert.equal(
    (
      await responseError(
        Response.json({ error: { message: "Deleted card cannot be restored" } }, { status: 400 }),
        "Update",
      )
    ).deleted,
    true,
  );
  const invalid = await responseError(
    Response.json({ error: { message: "token=secret" } }, { status: 400 }),
    "Update",
  );
...
test("Retry-After accepts dates and seconds; malformed JSON is explained", async () => {
  ...
  assert.equal(
    retryAfterMs(
      new Response(null, { headers: { "Retry-After": "Thu, 01 Jan 1970 00:00:10 GMT" } }),
      0,
    ),
    10000,
  );
